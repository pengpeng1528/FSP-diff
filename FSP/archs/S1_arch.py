...
def icnr(tensor, scale=2, init_method=init.kaiming_normal_):
    new_shape = [tensor.shape[0] // (scale ** 2)] + list(tensor.shape[1:])
    subkernel = torch.zeros(new_shape)
    init_method(subkernel)
    subkernel = subkernel.repeat_interleave(scale ** 2, 0)
    with torch.no_grad():
...
class Attention(nn.Module):
    ...
    def forward(self, x,k_v):
        ...
        # 数值稳定：在 fp32 下归一化并设置更大的 eps，避免 AMP 半精度下 eps 下溢为 0 导致除零
...
class Upsample(nn.Module):
    def __init__(self, n_feat):
        ...
        icnr(self.conv.weight, scale=2)
        self.body = nn.Sequential(self.conv, nn.PixelShuffle(2))
    def forward(self, x):
        return self.body(x)
