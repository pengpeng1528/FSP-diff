...
from FSP.data.data_util import (paired_paths_from_folder,
                                    paired_DP_paths_from_folder,
                                    paired_paths_from_lmdb,
                                    paired_paths_from_meta_info_file)
from FSP.data.transforms import augment, paired_random_crop, paired_random_crop_DP, random_augmentation
from FSP.utils import FileClient, imfrombytes, imfrommat, img2tensor, padding, padding_DP, imfrombytesDP
...
class DeblurPairedDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_samples):
            ...
            try:
                if gt_path.endswith('.mat'):
                    img_gt = imfrommat(gt_path, float32=True)
                else:
                    img_bytes = self.file_client.get(gt_path, 'gt')
                    img_gt = imfrombytes(img_bytes, float32=True)
            except:
                print(f"[警告] gt path {gt_path} 读取失败，跳过。")
                index += 1
                continue
            
            # 读取LQ图像
            try:
                if lq_path.endswith('.mat'):
                    img_lq = imfrommat(lq_path, float32=True)
                else:
                    img_bytes = self.file_client.get(lq_path, 'lq')
                    img_lq = imfrombytes(img_bytes, float32=True)
            except:
                print(f"[警告] lq path {lq_path} 读取失败，跳过。")
                index += 1
                continue
            ...
            # 如果所有样本都失败，抛出异常
            raise RuntimeError("所有样本均无法读取，请检查数据集！")

        # 假设输入已经是CHW（3*H*W），如果是HWC则需要转置
        if img_gt.ndim == 3 and img_gt.shape[0] != 3:
            img_gt = img_gt.transpose(2, 0, 1)
        if img_lq.ndim == 3 and img_lq.shape[0] != 3:
        ...
        # 按通道的联合min-max归一化：对每个通道c，用该样本(lq[c], gt[c])的共同min/max映射到[0,1]
        C = img_gt.shape[0]
        eps = 1e-6
        for c in range(C):
            cmin = min(float(np.min(img_lq[c])), float(np.min(img_gt[c])))
            cmax = max(float(np.max(img_lq[c])), float(np.max(img_gt[c])))
            denom = max(cmax - cmin, eps)
            img_lq[c] = (img_lq[c] - cmin) / denom
            img_gt[c] = (img_gt[c] - cmin) / denom
        # 安全裁剪
        img_lq = np.clip(img_lq, 0.0, 1.0)
        img_gt = np.clip(img_gt, 0.0, 1.0)

        # 不做BGR2RGB，不做HWC2CHW，直接转为tensor
