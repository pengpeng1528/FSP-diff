...
def paired_random_crop(img_gts, img_lqs, lq_patch_size, scale, gt_path):
    ...
    if img_lqs[0].ndim == 3 and img_lqs[0].shape[0] == 3:  # (C, H, W) 格式
        # 转换为 (H, W, C) 格式进行处理
        img_lqs = [img.transpose(1, 2, 0) for img in img_lqs]
        img_gts = [img.transpose(1, 2, 0) for img in img_gts]
    ...
    if need_transpose_back:
        img_lqs = [img.transpose(2, 0, 1) for img in img_lqs]
        img_gts = [img.transpose(2, 0, 1) for img in img_gts]
...
def random_augmentation(*args):
    # 检查输入格式并统一处理
    need_transpose_back = False
    if args[0].ndim == 3 and args[0].shape[0] == 3:  # (C, H, W) 格式
        # 转换为 (H, W, C) 格式进行处理
        args = [arg.transpose(1, 2, 0) for arg in args]
        need_transpose_back = True
    ...
    # 如果需要，转换回 (C, H, W) 格式
    if need_transpose_back:
        out = [arg.transpose(2, 0, 1) for arg in out]
