...
def imfrommat(file_path, float32=False):
    """Read an image from .mat file.
    ...
    try:
        ...
        # 查找包含图像数据的变量
        possible_keys = ['img', 'image', 'data', 'gt', 'clean', 'sharp', 'I', 'im']
        img = None
        ...
        if img is None:
            for key in mat_data.keys():
                if not key.startswith('__'):
                    ...
                    break
        if img is None:
            raise Exception(f'No image data found in {file_path}')
        img = np.array(img)
        # 处理数据类型和归一化
        if float32:
        ...
        if img.ndim == 3 and img.shape[-1] == 3:
            ...
            raise Exception(f'Unsupported image shape {img.shape} in {file_path}')
        return img
    except Exception as e:
        raise Exception(f'Failed to load .mat file {file_path}: {str(e)}')
...
def padding(img_lq, img_gt, gt_size):
    # 检查输入格式并统一处理
    if img_lq.ndim == 3 and img_lq.shape[0] == 3:  # (C, H, W) 格式
        # 转换为 (H, W, C) 格式进行处理
    ...
    if h_pad == 0 and w_pad == 0:
        if need_transpose_back:
            img_lq = img_lq.transpose(2, 0, 1)  # (H, W, C) -> (C, H, W)
