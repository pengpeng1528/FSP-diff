...
class FSPS2Model(SRModel):
    ...
    def __init__(self, opt):
        super(FSPS2Model, self).__init__(opt)
        if self.is_train:
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        self.is_train = False
        super(FSPS2Model, self).nondist_validation(dataloader, current_iter, tb_logger, save_img)
        self.is_train = True
    ...
    def optimize_parameters(self, current_iter):
        ...
        if current_iter < self.encoder_iter:
            ...
            if not torch.isfinite(l_total):
                print(f"[警告] 第{current_iter}步编码器损失出现非有限值，跳过该步更新。")
                return
            ...
            with autocast(enabled=False):
                ...
                # 新增：mse损失
                if hasattr(self, 'cri_mse') and self.cri_mse:
                    l_mse = self.cri_mse(pred32, gt32)
                    l_total = l_total + l_mse
                    loss_dict['l_mse'] = l_mse
                
                # 新增：ssim损失
                if hasattr(self, 'cri_ssim') and self.cri_ssim:
                ...
                S2_IPR=[pred_IPR_list[i]]
                l_kd, l_abs = self.cri_kd(S1_IPR, S2_IPR)
                l_total = l_total + l_abs
                loss_dict['l_kd_%d'%(i)] = l_kd
                loss_dict['l_abs_%d'%(i)] = l_abs
            
            # 非有限保护
            if not torch.isfinite(l_total):
                print(f"[警告] 第{current_iter}步SR损失出现非有限值，跳过该步更新。")
                return
